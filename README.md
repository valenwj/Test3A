# Test3A
Final Test AI Certification
This project evaluates the features that will have an impact on whether a person subscribe to a deposit. The features were the following: Input variables:
...
After the study the relevant features that have an impact on the subscription of the deposits are : age, job, education, default, duration, campaign, pdays, previous,emp.var.rate, cons.price.idx, cons.conf.idx and nr.employed. Logistic Regression provided the better accuracy metric while the KNeighborsClassifier() provided the better recall.
